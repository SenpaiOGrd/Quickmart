...
const Cart = () => {
    ...
    const  getCart = () => {
        ...
        for(const key in cartItems){
            const qty = Number(cartItems[key])
            if(!qty || qty <= 0) continue // skip items with zero/invalid quantity
            const found = products.find((item)=> item._id === key)
            if(!found) continue
            // clone product to avoid mutating source
            const product = {...found, quantity: qty}
            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product) => (
                    <div key={product._id} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <p className="text-center">{currency}{(product.offerPrice ?? product.offerprice) * product.quantity}</p>
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address,index)=>(<p key={index} onClick={() => {setSelectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">{address.street}, {address.city}, {address.state}, {address.country}</p>))}
